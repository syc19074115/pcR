...
def main():
    ...
    # tensorboard
    tb_writer = SummaryWriter('runs/test-7')  # 创立tensorboard，日志为空则其自动创立
    # optimizer
    ...
    for epoch in range(args.epochs - begin_epoch):
        ...
        # optimizer.state_dict()['param_groups'][0]['lr'] = learning_rate
        ...
        print("Train_loss = {}, Train_acc = {}".format(total_train_loss / len(train_dl), total_train_acc / len(train_dl)))
        ...
        if total_val_loss < best_loss:
            ...
            state = {
                ...
                'optimizer': optimizer.state_dict(),
                'epoch': epoch}
            save_name = os.path.join(save_path, "best.pth".format(epoch))
            torch.save(state, save_name)
