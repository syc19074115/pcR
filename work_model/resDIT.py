...
#########################
    ...
    def forward(self, before_x, after_x):
        ...
        x = self.dropout(x)
        x = self.transformer(x)
        #print(x.shape) #torch.Size([1, 64, 128])

        #做一个残差
        x = after_x + x[:,n:,:]
        return x[:,0]
        """
        #这段不要了，加一个post做encoder之后的残差
        if not self.use_scale:
            x = x.mean(dim=1) if self.pool == 'mean' else x[:, 0]
        else:
            scale = self.scale(x.mean(dim=-1)) #torch.Size([1, 64])-->[1,2]
