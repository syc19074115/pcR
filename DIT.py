...
class RearrangeImage1(nn.Module):
    ...
    def forward(self, x):
        return rearrange(x, 'b (d h w) c -> b c d h w', h=round(pow(int(x.shape[1]), 1 / 3)),
                         w=round(pow(int(x.shape[1]), 1 / 3)))
...
class DiT(nn.Module):
    def __init__(self, *,
        ...
        if self.pos_emb == 'share':
            ...
            self.pos_embedding_before_and_after = nn.Parameter(
                torch.randn(1, output_image_size ** 3 , dim))
        ...
        if self.use_scale:
            ...
            self.scale = nn.Sequential(
                nn.Linear(2 * output_image_size ** 3 , 2)
            )
    ...
    def forward(self, before_x, after_x):
        ...
        if not self.use_scale:
            ...
            # print(x.mean(dim=-1).shape)  #torch.Size([3, 2048])
